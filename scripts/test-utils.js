...
export function assertIncludes(array, value, message) {
    if (!array.includes(value)) {
        throw new AssertionError(message || `Expected array to include ${value}`);
    }
}
/**
...
export function assertContains(str, substr, message) {
    if (!str.includes(substr)) {
        throw new AssertionError(message || `Expected string to contain "${substr}"`);
    }
}
/**
...
export class DatabaseTestHelper {
     ...
     */
    static async airlineExists(code) {
        const query = `
     ...
     */
    static async aircraftExists(registration) {
        const query = `
     ...
     */
    static async getFleetCount(airlineCode) {
        const query = `
     ...
     */
    static async getAircraftTypeCount(aircraftType) {
        const query = `
      SELECT COUNT(*) as count
      FROM aircraft a
     ...
     */
    static async getPendingJobCount() {
     ...
     */
    static async getLatestJob(airlineCode) {
        const query = `
      SELECT sj.*
