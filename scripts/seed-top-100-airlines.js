...
class AirlineSeeder {
     ...
     */
    async cleanExistingData(dryRun) {
    ...
    validateAllData(airlines) {
        ...
        for (const airline of airlines) {
            try {
                ...
                logger.error(`Validation failed for ${airline.name}:`, error instanceof Error ? error.message : error);
                this.result.errors++;
            }
        }
        return valid;
     ...
     */
    async seedAirline(airline, dryRun) {
        this.result.total++;
        try {
            if (dryRun) {
                console.log(`  [DRY RUN] Would seed: ${airline.iata} - ${airline.name} (${airline.country})`);
      ...
      `;
            ...
            if (exists) {
                ...
                this.result.updated++;
                console.log(`  ✓ Updated: ${airline.iata} - ${airline.name} (${airline.country})`);
            }
            else {
                // Insert new airline
                await this.insertAirline(airline);
                this.result.inserted++;
                console.log(`  ✓ Inserted: ${airline.iata} - ${airline.name} (${airline.country})`);
            }
            this.updateStatistics(airline);
        }
        catch (error) {
            logger.error(`Failed to seed ${airline.name}:`, error instanceof Error ? error.message : error);
            this.result.errors++;
    ...
    printSummary() {
        ...
        if (this.result.errors === 0) {
            ...
            console.log(`⚠️  Seeding completed with ${this.result.errors} error(s)\n`);
        }
    }
    /**
...
//# sourceMappingURL=seed-top-100-airlines.js.map