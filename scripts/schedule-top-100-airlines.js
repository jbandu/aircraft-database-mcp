/**
 * Schedule Top 100 Airlines Script
 *
 * Initializes scraping jobs for the top 100 airlines by passenger traffic.
 * Sets up priorities and schedules based on airline size and importance.
 *
 * Usage:
 *   npm run scraper:setup
 *   npm run scraper:setup -- --dry-run
 *   npm run scraper:setup -- --force
 */
import { createLogger } from '../src/lib/logger.js';
import { queryPostgres } from '../src/lib/db-clients.js';
import { getJobQueue } from '../src/scrapers/workflows/job-queue.js';
const logger = createLogger('setup-scheduler');
/**
 * Top 100 airlines with their priorities
 * High priority: Major global carriers (top 30)
 * Normal priority: Regional and mid-size carriers (31-70)
 * Low priority: Smaller carriers (71-100)
 */
const TOP_100_AIRLINES = [
    // Top 30 - High Priority (Major Global Carriers)
    { code: 'AA', priority: 'high' },
    { code: 'DL', priority: 'high' },
    { code: 'UA', priority: 'high' },
    { code: 'WN', priority: 'high' },
    { code: 'BA', priority: 'high' },
    { code: 'LH', priority: 'high' },
    { code: 'AF', priority: 'high' },
    { code: 'KL', priority: 'high' },
    { code: 'EK', priority: 'high' },
    { code: 'QR', priority: 'high' },
    { code: 'SQ', priority: 'high' },
    { code: 'CZ', priority: 'high' },
    { code: 'CA', priority: 'high' },
    { code: 'MU', priority: 'high' },
    { code: 'NH', priority: 'high' },
    { code: 'JL', priority: 'high' },
    { code: 'QF', priority: 'high' },
    { code: 'AC', priority: 'high' },
    { code: 'TK', priority: 'high' },
    { code: 'AY', priority: 'high' },
    { code: 'IB', priority: 'high' },
    { code: 'AZ', priority: 'high' },
    { code: 'SU', priority: 'high' },
    { code: 'EY', priority: 'high' },
    { code: 'VS', priority: 'high' },
    { code: 'AS', priority: 'high' },
    { code: 'B6', priority: 'high' },
    { code: 'NK', priority: 'high' },
    { code: 'F9', priority: 'high' },
    { code: 'G4', priority: 'high' },
    // 31-70 - Normal Priority (Regional & Mid-Size Carriers)
    { code: 'LA', priority: 'normal' },
    { code: 'AM', priority: 'normal' },
    { code: 'CM', priority: 'normal' },
    { code: 'AR', priority: 'normal' },
    { code: 'AV', priority: 'normal' },
    { code: 'OS', priority: 'normal' },
    { code: 'SN', priority: 'normal' },
    { code: 'LX', priority: 'normal' },
    { code: 'SK', priority: 'normal' },
    { code: 'LO', priority: 'normal' },
    { code: 'OK', priority: 'normal' },
    { code: 'TP', priority: 'normal' },
    { code: 'UX', priority: 'normal' },
    { code: 'FR', priority: 'normal' },
    { code: 'U2', priority: 'normal' },
    { code: 'W6', priority: 'normal' },
    { code: 'VY', priority: 'normal' },
    { code: 'TP', priority: 'normal' },
    { code: 'EI', priority: 'normal' },
    { code: 'WS', priority: 'normal' },
    { code: 'TS', priority: 'normal' },
    { code: 'PD', priority: 'normal' },
    { code: 'AK', priority: 'normal' },
    { code: 'TR', priority: 'normal' },
    { code: 'GA', priority: 'normal' },
    { code: 'TG', priority: 'normal' },
    { code: 'VN', priority: 'normal' },
    { code: 'MH', priority: 'normal' },
    { code: 'PR', priority: 'normal' },
    { code: 'CI', priority: 'normal' },
    { code: 'BR', priority: 'normal' },
    { code: 'OZ', priority: 'normal' },
    { code: 'KE', priority: 'normal' },
    { code: 'HA', priority: 'normal' },
    { code: 'VA', priority: 'normal' },
    { code: 'JQ', priority: 'normal' },
    { code: 'NZ', priority: 'normal' },
    { code: 'SA', priority: 'normal' },
    { code: 'ET', priority: 'normal' },
    { code: 'MS', priority: 'normal' },
    // 71-100 - Low Priority (Smaller Carriers)
    { code: 'AT', priority: 'low' },
    { code: 'RO', priority: 'low' },
    { code: 'FB', priority: 'low' },
    { code: 'PC', priority: 'low' },
    { code: 'TU', priority: 'low' },
    { code: 'RJ', priority: 'low' },
    { code: 'GF', priority: 'low' },
    { code: 'WY', priority: 'low' },
    { code: 'SV', priority: 'low' },
    { code: 'FZ', priority: 'low' },
    { code: '6E', priority: 'low' },
    { code: 'AI', priority: 'low' },
    { code: 'SG', priority: 'low' },
    { code: 'UK', priority: 'low' },
    { code: 'QZ', priority: 'low' },
    { code: 'FD', priority: 'low' },
    { code: 'BI', priority: 'low' },
    { code: 'DD', priority: 'low' },
    { code: 'SL', priority: 'low' },
    { code: 'UL', priority: 'low' },
    { code: 'BG', priority: 'low' },
    { code: 'KC', priority: 'low' },
    { code: 'HU', priority: 'low' },
    { code: 'SC', priority: 'low' },
    { code: 'FM', priority: 'low' },
    { code: 'ZH', priority: 'low' },
    { code: 'MF', priority: 'low' },
    { code: '3U', priority: 'low' },
    { code: 'GS', priority: 'low' },
    { code: 'HO', priority: 'low' },
];
export class AirlineSchedulerSetup {
    jobQueue = getJobQueue();
    /**
     * Set up scraping jobs for top 100 airlines
     */
    async setup(options) {
        logger.info('Setting up scraping jobs for top 100 airlines', options);
        let created = 0;
        let skipped = 0;
        let failed = 0;
        for (let i = 0; i < TOP_100_AIRLINES.length; i++) {
            const airline = TOP_100_AIRLINES[i];
            try {
                // Check if airline exists in database
                const airlineData = await this.getAirlineInfo(airline.code);
                if (!airlineData) {
                    logger.warn(`Airline not found in database: ${airline.code}`);
                    skipped++;
                    continue;
                }
                // Check if job already exists (unless force)
                if (!options?.force) {
                    const existingJob = await this.hasRecentJob(airline.code);
                    if (existingJob) {
                        logger.debug(`Job already exists for ${airline.code}, skipping`);
                        skipped++;
                        continue;
                    }
                }
                // Calculate scheduled delay (spread out jobs over time)
                const scheduledDelay = this.calculateScheduledDelay(i, airline.priority);
                const setup = {
                    iata_code: airline.code,
                    name: airlineData.name,
                    priority: airline.priority,
                    scheduledDelay,
                    maxRetries: airline.priority === 'high' ? 5 : 3,
                    retryDelayMinutes: 30,
                };
                if (options?.dryRun) {
                    logger.info(`[DRY RUN] Would create job for ${setup.iata_code} (${setup.name}) - Priority: ${setup.priority}, Delay: ${setup.scheduledDelay}min`);
                    created++;
                }
                else {
                    const jobId = await this.createJob(setup);
                    logger.info(`Created job ${jobId} for ${setup.iata_code} (${setup.name}) - Priority: ${setup.priority}, Scheduled in ${setup.scheduledDelay}min`);
                    created++;
                }
                // Small delay between creations
                await this.sleep(100);
            }
            catch (error) {
                logger.error(`Failed to create job for ${airline.code}:`, error);
                failed++;
            }
        }
        logger.info(`Setup complete: ${created} jobs created, ${skipped} skipped, ${failed} failed`);
    }
    /**
     * Get airline information from database
     */
    async getAirlineInfo(airlineCode) {
        const query = `
      SELECT id, name
      FROM airlines
      WHERE UPPER(iata_code) = UPPER($1) OR UPPER(icao_code) = UPPER($1)
      LIMIT 1
    `;
        const result = await queryPostgres(query, [airlineCode]);
        return result.rows[0] || null;
    }
    /**
     * Check if airline has a recent job
     */
    async hasRecentJob(airlineCode) {
        const query = `
      SELECT id
      FROM scraping_jobs sj
      JOIN airlines al ON sj.airline_id = al.id
      WHERE (UPPER(al.iata_code) = UPPER($1) OR UPPER(al.icao_code) = UPPER($1))
        AND sj.status IN ('pending', 'running')
      LIMIT 1
    `;
        const result = await queryPostgres(query, [airlineCode]);
        return result.rows.length > 0;
    }
    /**
     * Calculate scheduled delay for job
     * Spreads jobs over time to avoid overload
     */
    calculateScheduledDelay(index, priority) {
        // High priority: start immediately or within 1 hour
        // Normal priority: spread over 2-6 hours
        // Low priority: spread over 6-12 hours
        let baseDelay;
        let spread;
        switch (priority) {
            case 'high':
                baseDelay = 0;
                spread = 60; // 1 hour
                break;
            case 'normal':
                baseDelay = 120;
                spread = 240; // 4 hours
                break;
            case 'low':
                baseDelay = 360;
                spread = 360; // 6 hours
                break;
        }
        // Calculate delay based on index within priority group
        const delay = baseDelay + (index * spread) / TOP_100_AIRLINES.length;
        return Math.floor(delay);
    }
    /**
     * Create a scraping job
     */
    async createJob(setup) {
        const scheduledAt = new Date(Date.now() + setup.scheduledDelay * 60 * 1000);
        return await this.jobQueue.createJob(setup.iata_code, {
            jobType: 'full_fleet_update',
            priority: setup.priority,
            maxRetries: setup.maxRetries,
            retryDelayMinutes: setup.retryDelayMinutes,
            scheduledAt,
            metadata: {
                initial_setup: true,
                airline_name: setup.name,
            },
        });
    }
    /**
     * Sleep helper
     */
    sleep(ms) {
        return new Promise((resolve) => setTimeout(resolve, ms));
    }
    /**
     * Update airline scrape settings
     */
    async updateAirlineSettings(airlineCode, settings) {
        const updates = [];
        const params = [];
        let paramIndex = 1;
        if (settings.scrapeEnabled !== undefined) {
            updates.push(`scrape_enabled = $${paramIndex++}`);
            params.push(settings.scrapeEnabled);
        }
        if (settings.scrapeScheduleCron) {
            updates.push(`scrape_schedule_cron = $${paramIndex++}`);
            params.push(settings.scrapeScheduleCron);
        }
        if (updates.length === 0) {
            return;
        }
        params.push(airlineCode);
        const query = `
      UPDATE airlines
      SET ${updates.join(', ')}
      WHERE UPPER(iata_code) = UPPER($${paramIndex}) OR UPPER(icao_code) = UPPER($${paramIndex})
    `;
        await queryPostgres(query, params);
        logger.info(`Updated settings for ${airlineCode}`);
    }
}
// CLI support
if (import.meta.url === `file://${process.argv[1]}`) {
    const args = process.argv.slice(2);
    const dryRun = args.includes('--dry-run');
    const force = args.includes('--force');
    logger.info('Aircraft Database Scraper - Setup Script');
    logger.info(`Mode: ${dryRun ? 'DRY RUN' : 'LIVE'}`);
    logger.info(`Force: ${force ? 'YES' : 'NO'}\n`);
    const setup = new AirlineSchedulerSetup();
    setup
        .setup({ dryRun, force })
        .then(() => {
        logger.info('\n✅ Setup completed successfully');
        process.exit(0);
    })
        .catch((error) => {
        logger.error('Setup failed:', error);
        process.exit(1);
    });
}
//# sourceMappingURL=schedule-top-100-airlines.js.map